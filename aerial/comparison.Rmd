...
scott_new <- scott %>% 
  dplyr::transmute(
    scott_species = species,
    scott_nest_age = nest_age,
    wastd_id = wastd_id
  )

tony_new <- tony %>% 
  dplyr::transmute(
    tony_species = species,
    tony_nest_age = nest_age,
    wastd_id = wastd_id
  )

wastd_and_scott <- wastd_new %>% 
  dplyr::left_join(scott_new, by = "wastd_id") %>%
  dplyr::mutate(
    scott_species_ok = ifelse(w_species == scott_species, 1, 0),
    scott_nest_age_ok = ifelse(w_nest_age == scott_nest_age, 1, 0)
  )
DT::datatable(wastd_and_scott, caption = "Tracks seen by Scott and WAStD")
summary(wastd_and_scott[,8:9])

scott_not_wastd <- scott %>%
  dplyr::anti_join(wastd_new, by = "wastd_id")
DT::datatable(scott_not_wastd, caption = "Tracks seen by Scott not in WAStD")

wastd_and_tony <- wastd_new %>% 
  dplyr::full_join(tony_new, by = "wastd_id") %>%
  dplyr::mutate(
    tony_species_ok = ifelse(w_species == tony_species, 1, 0),
    tony_nest_age_ok = ifelse(w_nest_age == tony_nest_age, 1, 0)
  )
