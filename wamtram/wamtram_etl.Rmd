...
# Read in all data from W2
entry_batches <- DBI::dbReadTable(con, "TRT_ENTRY_BATCHES")
data_entry <- DBI::dbReadTable(con, 'TRT_DATA_ENTRY')
data_entry_operators <- DBI::dbReadTable(con, 'TRT_DATA_ENTRY_PERSONS')
sites <- DBI::dbReadTable(con, 'TRT_PLACES') %>%
  dplyr::rename(
...
tag_states <- DBI::dbReadTable(con, 'TRT_TAG_STATES')

# Sensitive information!
persons <- DBI::dbReadTable(con, 'TRT_PERSONS')
...
activities <- DBI::dbReadTable(con, 'TRT_ACTIVITIES') %>%
  transmute(
...
datum_codes <- DBI::dbReadTable(con, 'TRT_DATUM_CODES')

# Tag types
id_types <- DBI::dbReadTable(con, 'TRT_IDENTIFICATION_TYPES') %>%
  transmute(
    id_type_code=IDENTIFICATION_TYPE,
    id_type_label=DESCRIPTION)

# 220k+ records
recorded_tags <- DBI::dbReadTable(con, 'TRT_RECORDED_TAGS') %>%
  transmute(
...
# 55k+ inferred turtle identities, reconstructed from encounters.
turtles <- DBI::dbReadTable(con, 'TRT_TURTLES')

# Synthesised information about tags, reconstructed from recorded_tags and other tag asset management operations.
tags <- DBI::dbReadTable(con, 'TRT_TAGS') %>% arrange(desc(TAG_ID))

samples <- DBI::dbReadTable(con, 'TRT_SAMPLES')
sample_tissue_type <- DBI::dbReadTable(con, 'TRT_TISSUE_TYPES')

pit_tags <- DBI::dbReadTable(con, 'TRT_PIT_TAGS')
recorded_pit_tags <- DBI::dbReadTable(con, 'TRT_RECORDED_PIT_TAGS')
pit_tag_states <- DBI::dbReadTable(con, 'TRT_PIP_TAG_STATES')

measurement_types <- DBI::dbReadTable(con, 'TRT_MEASUREMENT_TYPES') %>%
  transmute(
...
measurements <- DBI::dbReadTable(con, 'TRT_MEASUREMENTS') %>%
  transmute(
    observation_id=OBSERVATION_ID,
    measurement_type_code=MEASUREMENT_TYPE,
...
damage_codes <- DBI::dbReadTable(con, 'TRT_DAMAGE_CODES')
damage_causes <- DBI::dbReadTable(con, 'TRT_DAMAGE_CAUSE_CODES')
damages <- DBI::dbReadTable(con, 'TRT_DAMAGE')

# 149k+
obs <- DBI::dbReadTable(con, 'TRT_OBSERVATIONS')
