...
---
```{r, message=F}
setwd("~/projects/turtle-scripts/wamtram")
library(Hmisc)
library(tidyr)
library(dplyr)
library(stringr)
...
* in Windows: ask IT to configure the ODBC data source in Control Panel > ODBC Data Sources,
* Apple: going to the Apple store and buying a new MacBook Pro, or
* Linux: writing four lines to a file `~/.odbc.ini`:
...
sites_with_coordinates <- sites %>% filter(!is.na(site_latitude))
...
person_names <- persons %>%
    transmute(
        PERSON_ID=PERSON_ID,
        name=paste(FIRST_NAME, SURNAME) %>% str_replace_all("[<>()*=?]", "") %>% str_trim(),
        email=EMAIL %>% str_replace("NA", "") %>% str_trim())
person_details <- persons %>%
    mutate(
        name=paste(FIRST_NAME, SURNAME) %>% str_replace_all("[<>()*=?]", "") %>% str_trim(),
        email=EMAIL %>% str_replace("NA", "") %>% str_trim())
write.csv(person_details, file="data/wamtram_users.csv", row.names=F)
...
write.csv(recorded_tags, file="data/wamtram_tagobservations.csv", row.names=F)
...
o <- obs %>%
    ...
    select(
        ...
        -CORRECTED_DATE, -DATE_CONVENTION, -o_date, -o_time
    ) %>%
...
enc <- o %>%
    filter(!is.na(LONGITUDE) & !is.na(observation_datetime_utc)) %>%
    filter(PLACE_CODE == "THEE" | PLACE_CODE == "THNB")

datatable(enc)
# write.csv(o, file="data/wamtram_encounters.csv", row.names=F)
write.csv(enc, file="data/wamtram_encounters.csv", row.names=F)
ckanr::resource_update(W2_ENC_CSV_RID, "data/wamtram_encounters.csv")
```
