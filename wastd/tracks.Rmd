...
tracks <- data_trk_json %>%
  ...
  filter(
    latitude < -21.44 & latitude > -21.47 & longitude > 114.96 & longitude < 115.03
  )
...
# TODO get nesting success for tags from "eggs laid"
# TODO bind_rows(tracks, tags) incl nesting success, exclude "old" tracks
...
# Nesting effort
**coming soon** Nesting turtles will be included. Currently, only tracks / nests
are shown from here.
...
ggsave("data/nesting_abundance.pdf", gg_nesting_abundance, width=7, height=10)
```
...
gg_nesting_success <- ggplot(nesting_success, aes(x=date, y=nesting_success)) +
  ...
  facet_wrap("species", ncol = 1)
gg_nesting_success
ggsave("data/nesting_success.pdf", gg_nesting_success, width=7, height=10)
```
