...
# Very fast through vectorisation: flatten nested list into tbl_df
...
tags <- ldply(data_tag_json, flatten_tag) %>% tbl_df()
...
data_retrieved_on <- with_tz(parse_date_time(res_trk$headers$date, orders=ord, tz=utc), tzone=gmt08)
no_observations <- nrow(res_trk)
earliest_observation <- with_tz(min(tracks$datetime), tzone=gmt08)
latest_observation <- with_tz(max(tracks$datetime), tzone=gmt08)
...
# Nesting effort
TODO: include tagging from here on.
