...
### How often do turtles come back?
...
### How many days elapse to first resighting?
...
### What is the hatching and emergence success?
...
leaflet(filter(nests, hatching_success >= 0, species == "natator-depressus")) %>% 
  addProviderTiles("Esri.WorldImagery", group = "Aerial") %>%
  addProviderTiles("OpenStreetMap.Mapnik", group = "Place names") %>%
  setView(lng=115.0, lat=-21.45, zoom=12) %>%
...
## Nesting effort from tracks and tags combined
...
### Nesting effort by species over time (tracks and tags)

**TODO** Tally `nesting_encounters` per day and species, plot n over date
...
Now we can summarise all nesting events in different combinations of successful 
and non-successful event types over the whole season (all observed dates) and plot by species.

We'll define one plotting function `ggplot_nesting_distribution` to factor out
repeating code.

```{r plot_nesting_distribution}
ggplot_nesting_distribution <- function(data, grid,
                                        spe="natator-depressus", 
                                        typ=c("track-not-assessed", 
                                              "track-unsure", 
                                              "successful-crawl", 
                                              "false-crawl", 
                                              "nest",
  ...
  data %>% 
    ...
    ggplot(aes(xmin, ymin, z = n)) + 
    geom_density2d(color = "grey") + 
    geom_raster(aes(fill = n)) +
    scale_fill_gradientn(
      colours = c("blue", "green", "yellow", "orange", "red")) +
    ggtitle(paste(stringr::str_to_title(sel), 
                  "nesting events over whole season:", 
                  spe %>% humanize)) + 
    labs(x = "Longitude", y = "Latitude") + 
    coord_fixed() +
    theme_light() +
    ggsave(
      file = paste0("data/map_", sel, "_nesting_", spe, ".pdf"), 
      width = 7, height = 5)
}

d <- d.spdf.grd@data

# Tally observations by grid cell, species over all dates
ggplot_nesting_distribution(d, Xs.id)
ggplot_nesting_distribution(d, Xs.id, spe = "chelonia-mydas")

# Only successful tracks ("successful-crawl") and tags (nesting "present")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "natator-depressus", 
                            typ = c("successful-crawl", "present"), 
                            sel = "successful")
...
# Only unsuccessful tracks ("false-crawl", "track-unsure", "track-not-assessed") 
# and tags without confirmed nesting success (nesting "absent" or "na")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "natator-depressus", 
                            typ = c("false-crawl", "track-unsure", "track-not-assessed", "absent", "na"), 
                            sel = "unsuccessful")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "chelonia-mydas", 
                            typ = c("false-crawl", "track-unsure", "track-not-assessed", "absent", "na"), 
...
# same for only successful nesting (nesting "present")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "natator-depressus", 
                            typ = c("present"), 
                            sel = "nests from tagging")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "chelonia-mydas", 
                           typ = c("present"), 
                            sel = "nests from tagging")
...
# same for only unsuccessful nesting (nesting not "present")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "natator-depressus", 
                            typ = c("absent", "na"), 
                            sel = "unsuccessful tagged")
ggplot_nesting_distribution(d, Xs.id,
                            spe = "chelonia-mydas", 
                           typ = c("absent", "na"), 
                            sel = "unsuccessful tagged")
...
species_profiles_areas <- d.spdf.grd@data %>% 
  ...
  mutate(
    ...
    percent_in_tagging_area = 100 * (tagging_area / (tagging_area + outside)) %>% round(digits = 3)
  )
datatable(species_profiles_areas, 
          caption = "Number of tracks and taggings in tagging area vs all",
          options = list(paging = F))
```
...
### Spatial and temporal patterns in total nesting
Outcome: 
