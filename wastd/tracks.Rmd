...
nesting_success_map <- leaflet(filter(nests, hatching_success >= 0, species == "natator-depressus")) %>% 
  addProviderTiles("Esri.WorldImagery", group = "Aerial") %>%
  addProviderTiles("OpenStreetMap.Mapnik", group = "Place names") %>%
...
nesting_success_map
mapshot(nesting_success_map, file = "nesting_success_map.png")
```
...
## Nesting success by species (tracks and tags excl old nests)

```{r}
fresh_nesting <- nesting_encounters %>%
  filter(type %in% c("successful-crawl", 
                     "false-crawl", 
                     "track-unsure", 
                     "track-not-assessed"))

fresh_nesting_by_date <- fresh_nesting %>%
  group_by(species, date) %>% tally %>% ungroup %>% rename(all = n)

successful_nesting_by_date <- fresh_nesting %>% 
  filter(type %in% c("successful-crawl", "present")) %>%
  group_by(date, species) %>% tally() %>% ungroup() %>% rename(successful = n)

# Join nesting emergences with nesting success or 0
tt_nesting_success <- fresh_nesting_by_date %>%
  left_join(successful_nesting_by_date, by = c('date','species')) %>%
  mutate(successful = ifelse(is.na(successful), 0, successful),
         nesting_success = 100 * successful/all)

ggplot(tt_nesting_success, aes(date, nesting_success)) + 
  geom_bar(stat = "identity") + 
  facet_wrap(~species) +
  scale_x_date(breaks = scales::pretty_breaks(),
                 labels = scales::date_format("%d %b %Y")) +
    labs(x = "Date", y = "Success rate") +
    ggtitle(paste("Nesting success rate"),
            subtitle = "Fraction of successful over total nesting tracks and taggings") +
    theme_light() +
    ggsave(paste0("data/track_and_tag_nesting_success_rate.pdf"), 
           width = 7, height = 5)

tt_nesting_success_summary <- tt_nesting_success %>%
  group_by(species) %>%
  dplyr::summarize(
    "observed_days" = n(),
    ...
    "nesting_success_rate_mean" = mean(nesting_success) %>% round(digits = 2),
    "nesting_success_rate_sd" = sd(nesting_success) %>% round(digits = 2),
    "nesting_success_rate_min" = min(nesting_success) %>% round(digits = 2),
    "nesting_success_rate_max" = max(nesting_success) %>% round(digits = 2)
    )

DT::datatable(tt_nesting_success_summary, 
