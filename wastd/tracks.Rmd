...
Xs.id <- grd@data %>% filter(id %in% unique(dd$id)) %>% dplyr::select(id, xmin, ymin)
Xs <- Xs.id %>% dplyr::select(-id)
plot(Xs)

Xt <- difftime(dd$date, origin)
Y.in <- dd %>% dplyr::select(-id, -date, -xmin, -xmax, -ymin, -ymax)
X.coords <- dd %>% dplyr::select(xmin, ymin)
```
...
plot(Xs)
Xs.dist <- dist(Xs)
# TODO: discard all but nearest neighbour
# Xs.dist[which(Xs.dist>4*min(Xs.dist))]
# X.spantree <- spantree(Xs.dist)
Xs.site.pcnm <- PCNM(Xs.dist)
# Truncation level = 0.001415214 
# Time to compute PCNMs = 3045.977000  sec = 50 min -- need to optimize
...
Xt.dist <- dist(Xt)
Xt.pcnm <- PCNM(Xt.dist)
Xt.pcnm.pos <- as.data.frame(Xt.pcnm$vectors)[,which(Xt.pcnm$Moran_I$Positive == T)]
Xt.scores <- as.data.frame(scores(Xt.pcnm)) # use .pos once PCNM installed
