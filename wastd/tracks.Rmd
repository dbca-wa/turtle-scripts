...
tracks <- track_records$content %>% 
  mutate(
    ...
    hatching_success =  properties.observation_set %>% 
      map(get_num_field, "hatching_success"),
    emergence_success = properties.observation_set %>% 
      map(get_num_field, "emergence_success")
  ) %>% thv_filter 
...
# TODO extract from lists of lists
# extract_field <- function(lol, field){
#   if (length(lol) == 0) return(NA)
#   oo <- set_names(lol, map_chr(lol, "observation_name"))
#   if ("turtlenestobservation" %in% names(oo)) {
#     return(extract2(oo$turtlenestobservation, field))
#   } else return(NA)
# }
# 
# #' Get a numeric field from an unnamed list of lists
# get_field <- function(lol, field) {
#   if (length(lol) == 0) return(NA)
#   cat("Operating on list of lists of length", length(lol))
#   l1 <- map(lol, c("properties", "observation_set")) %>% map(., extract_field(l1, field))
# }
# 
# get_field(nest_json$content, "hatching_success")
...
nests <- nest_json$content %>% {
  tibble::tibble(
    ...
    turtle_date = map_chr(., c("properties", "when")) %>% as_turtle_date,
    species = map_chr(., c("properties", "species")),
    obs = map(., c("properties", "observation_set"))
    )
} 
# We will use nests once hs and es are extracted
...
```{r}
nests <- tracks %>% 
  filter(hatching_success>-1) %>% 
  mutate(
    hatching_success = as.numeric(hatching_success),
    emergence_success = as.numeric(emergence_success)
  ) %>%
  select(datetime, date, longitude, latitude, species, hatching_success, emergence_success)
DT::datatable(nests)
```

```{r}
ggplot(nests) + 
  geom_boxplot(aes(group=date, x=date, y=as.numeric(hatching_success)), color="black") +
  geom_boxplot(aes(group=date, x=date, y=as.numeric(emergence_success)), color="gray") +
  xlab("Date") + ylab("Success rate in percent") + ggtitle("Hatching and emergence success") +
  theme_minimal()
```
...
leaflet(nests) %>% 
  ...
  addAwesomeMarkers(~longitude, ~latitude,
                    ...
                    data=nests, label = ~species,
                    icon = makeAwesomeIcon(icon = "home", markerColor = "green"),
                    popup = ~paste(species, datetime, "HS", hatching_success, "ES", emergence_success),
                    group = "Nests") %>%
  addLayersControl(baseGroups = c("Aerial", "Place names"),
