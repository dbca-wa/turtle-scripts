...
ggplot(tags_by_date, aes(x = date, y = n)) + geom_bar(stat = "identity") +
  ...
  ggsave("data/turtles_processed_per_day.pdf", width = 7, height = 5)
```
...
ggplot(nests_with_hs, aes(group=date, x=date)) + 
  geom_boxplot(aes(y=as.numeric(hatching_success)), na.rm=TRUE, show.legend=TRUE, color="black") +
  geom_boxplot(aes(y=as.numeric(emergence_success)), na.rm=TRUE, show.legend=TRUE, color="gray") +
  ggtitle("Flatback hatching (black) and emergence (grey) success") +
  xlab("Date") + ylab("Success rate in percent") + theme_minimal()
```


Mean, SD and range for HS and ES and clutch size per species:
...
DT::datatable(success_summary, caption = "Nesting success summary")
```

### Spatial distribution of hatching success

Size correlates to clutch size, colour to hatching success.
...
palette = rainbow(10, start=rgb2hsv(col2rgb('red'))[1], end=rgb2hsv(col2rgb('green'))[1])

pal <- colorBin(palette, nests_with_hs$hatching_success, 10, pretty = FALSE)

leaflet(filter(nests, hatching_success >= 0)) %>% 
  addProviderTiles("Esri.WorldImagery", group = "Aerial") %>%
  addProviderTiles("OpenStreetMap.Mapnik", group = "Place names") %>%
  setView(lng=115.0, lat=-21.45, zoom=12) %>%
  addPolygons(data = tagging_area, weight = 1, color = "blue", opacity = 0.5, 
...
`r length(unique(tracks$date))` days were surveyed.
...
datatable(tracks_by_species, caption = "Tracks per day")
```
...
```{r plot_nesting_abundance, fig.height=7, fig.width=7}
...
ggplot(nesting_success, aes(x=date)) +
  ...
  ggtitle("Nesting effort at Thevenard Island", 
          subtitle="Green: tracks with nest, nesting turtles\nBlack: all nesting") +
  labs(x="Date", y="Number of all and successful nesting events") +
  facet_wrap("species", ncol=1) +
  ggsave(file="data/nesting_abundance.pdf", width=7, height=7)
```

```{r nesting_success_fb}
ggplot(nesting_success %>% filter(species=="natator-depressus"), aes(x=date)) +
  geom_bar(aes(y=all), stat="identity", color="black", fill="black") +
  geom_bar(aes(y=successful), stat="identity", color="green", fill="green") +
  ggtitle("Nesting effort at Thevenard Island: Flatbacks", 
          subtitle="Green: tracks with nest, nesting turtles\nBlack: all nesting") +
  labs(x="Date", y="Number of all and successful nesting events") +
  facet_wrap("species", ncol=1) +
  ggsave(file="data/nesting_abundance_fb.pdf", width=7, height=7)
```
```{r nesting_success_gn}
ggplot(nesting_success %>% filter(species=="chelonia-mydas"), aes(x=date)) +
  geom_bar(aes(y=all), stat="identity", color="black", fill="black") +
  geom_bar(aes(y=successful), stat="identity", color="green", fill="green") +
  ggtitle("Nesting effort at Thevenard Island: Greens", 
          subtitle="Green: tracks with nest, nesting turtles\nBlack: all nesting") +
  labs(x="Date", y="Number of all and successful nesting events") +
  facet_wrap("species", ncol=1) +
  ggsave(file="data/nesting_abundance_gn.pdf", width=7, height=7)
```

TODO: Fitting a GAM to the nesting activity curves would model the nesting season.
...
ggplot(site_summary %>% filter(species=="natator-depressus"), aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
  geom_density2d(color="grey") + 
  geom_raster(aes(fill=n)) +
  scale_fill_gradientn(colours=c("blue","green","yellow","orange","red")) +
  ggtitle("Observed nesting events over whole season: Flatbacks") + 
  labs(x="Longitude", y="Latitude") + coord_fixed() +
  ggsave(file="data/nesting_distribution_all_time_fb.pdf", width=7, height=5)

ggplot(site_summary %>% filter(species=="chelonia-mydas"), aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
  geom_density2d(color="grey") + 
  geom_raster(aes(fill=n)) +
  scale_fill_gradientn(colours=c("blue","green","yellow","orange","red")) +
  ggtitle("Observed nesting events over whole season: Greens") + 
  labs(x="Longitude", y="Latitude") + coord_fixed() +
  ggsave(file="data/nesting_distribution_all_time_gn.pdf", width=7, height=5)


# same for only successful tracks ("successful-crawl") and tags (nesting "present")
site_summary_successful_nesting <- d.spdf.grd@data %>% 
  filter(!is.na(id), type %in% c("successful-crawl", "present")) %>%
  group_by(id, species, name) %>% 
  tally(sort=F) %>% 
  ungroup() %>%
  left_join(Xs.id,by = "id")

ggplot(site_summary_successful_nesting %>% filter(species=="natator-depressus"), aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
  geom_density2d(color="grey") + 
  geom_raster(aes(fill=n)) +
  scale_fill_gradientn(colours=c("blue","green","yellow","orange","red")) +
  ggtitle("Observed successful nesting events over whole season: Flatbacks") + 
  labs(x="Longitude", y="Latitude") + coord_fixed() +
  ggsave(file="data/nesting_distribution_all_time_fb_successful.pdf", width=7, height=5)


ggplot(site_summary_successful_nesting %>% filter(species=="chelonia-mydas"), aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
  geom_density2d(color="grey") + 
  geom_raster(aes(fill=n)) +
  scale_fill_gradientn(colours=c("blue","green","yellow","orange","red")) +
  ggtitle("Observed successful nesting events over whole season: Greens") + 
  labs(x="Longitude", y="Latitude") + coord_fixed() +
  ggsave(file="data/nesting_distribution_all_time_gn_successful.pdf", width=7, height=5)

# same for only unsuccessful tracks ("false-crawl", "track-unsure", "track-not-assessed") and tags (nesting not "present")
# same for only successful tracks ("successful-crawl") and tags (nesting "present")
site_summary_unsuccessful_nesting <- d.spdf.grd@data %>% 
  filter(!is.na(id), type %in% c("false-crawl", "track-unsure", "track-not-assessed", "absent")) %>%
  group_by(id, species, name) %>% 
  tally(sort=F) %>% 
  ungroup() %>%
  left_join(Xs.id,by = "id")

ggplot(site_summary_unsuccessful_nesting %>% filter(species=="natator-depressus"), aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
  geom_density2d(color="grey") + 
  geom_raster(aes(fill=n)) +
  scale_fill_gradientn(colours=c("blue","green","yellow","orange","red")) +
  ggtitle("Observed unsuccessful nesting events over whole season: Flatbacks") + 
  labs(x="Longitude", y="Latitude") + coord_fixed() +
  ggsave(file="data/nesting_distribution_all_time_fb_unsuccessful.pdf", width=7, height=5)


ggplot(site_summary_unsuccessful_nesting %>% filter(species=="chelonia-mydas"), aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
  geom_density2d(color="grey") + 
  geom_raster(aes(fill=n)) +
  scale_fill_gradientn(colours=c("blue","green","yellow","orange","red")) +
  ggtitle("Observed unsuccessful nesting events over whole season: Greens") + 
  labs(x="Longitude", y="Latitude") + coord_fixed() +
  ggsave(file="data/nesting_distribution_all_time_gn_unsuccessful.pdf", width=7, height=5)
...
### How much nesting happens in the tagging area?
...
species_profiles_areas <- d.spdf.grd@data %>% 
  ...
  mutate(
    ...
    percent_in_tagging_area = (tagging_area / (tagging_area + outside)) %>% round(digits = 2)
  )
datatable(species_profiles_areas, caption = "Number of tracks and taggings in tagging area vs all")
```


* Proportion of tracks in the tagging area compared to the rest of the island for flatbacks
* Graph: Temporal distribution of tracks (all and successful) per species (flat and green)
* Graph: Spatial distribution of tracks around the island, per species (flat and green)
* Graph: Spatial distribution of nests around the island for flatbacks (kernel density?)
* Graph: Spatial distribution of false crawls around the island for flatbacks (kernel density?)
