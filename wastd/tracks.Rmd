...
# Flatten nested JSON to data.table using tidyjson: slow but more readable
# tracks <- data_trk %>%
...
track_data_retrieved_on <- with_tz(
  parse_date_time(res_trk$headers$date, orders=ord, tz=utc), tzone=gmt08)
...
tag_data_retrieved_on <- with_tz(
  parse_date_time(res_tag$headers$date, orders=ord, tz=utc), tzone=gmt08)
...
turtle activity (from tagging) are combined into one column.

**TODO** Nesting success from tagging is not yet loaded from the original data,
but will replace turtle activity with the variable "nesting success".

Furthermore, turtle dates are calculated for all encounters.
