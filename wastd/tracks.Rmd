...
tracks <- track_records$content %>% 
  mutate(
    ...
    hatching_success =  properties.observation_set %>% map(get_num_field, "hatching_success"),
    emergence_success = properties.observation_set %>% map(get_num_field, "emergence_success")
  ) %>% thv_filter 
...
```{r}
nests <- tracks %>% filter(hatching_success>-1)
nests[1,]$hatching_success %>% as.numeric
nests %>% mutate(hs = as.numeric(hatching_success))
```
