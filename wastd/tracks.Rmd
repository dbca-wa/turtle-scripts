...
extract_possibly <- possibly(extract, otherwise = NA_real_)
get_f <- function(lol, field) lol %>% unlist %>% extract_possibly(field) %>% unlist %>% as.numeric

get_ff <- function(lol, field) {
  purrr::when(
    length(lol) == 0 ~ NA_real_,
    ~ lol %>% unlist %>% extract_possibly(field) %>% unlist %>% as.numeric
  )
  # TODO replace numeric(0) with NA_real_
  }
...
tracks <- track_records$content %>% 
  mutate(
    ...
    hatching_success =  obs %>% map(get_ff, "hatching_success"),
    emergence_success = obs %>% map(get_ff, "emergence_success")
  ) %>% thv_filter 
...
trackIcons <- awesomeIconList(
  "cheloniidae-fam" = makeAwesomeIcon(icon='home', library='glyphicon', markerColor = 'black'),
  "chelonia-mydas" = makeAwesomeIcon(icon='home', library='glyphicon', markerColor = 'green'),
  "eretmochelys-imbricata" = makeAwesomeIcon(icon='home', library='glyphicon', markerColor = 'blue'),
  "natator-depressus" = makeAwesomeIcon(icon='home', library='glyphicon', markerColor = 'red')
  )
...
leaflet(tracks) %>% 
  ...
  addLayersControl(baseGroups = c("Aerial", "Place names"),
                   overlayGroups = c("Grid", "Tracks", "Tags"))
...
## Does nesting happen in the tagging area?
...
### Hatching and emergence success over time
...
ggplot(tracks) + 
  ...
  ggtitle("Hatching (black) and emergence (grey) success") +
  xlab("Date") + ylab("Success rate in percent") + theme_minimal()
```

### Spatial distribution of excavated nests
...
leaflet(nests) %>% 
  ...
  addAwesomeMarkers(~longitude, ~latitude,
                    data = nests, 
                    icon = ~popIcons[species],
                    label = ~paste(date, species),
                    popup = ~paste(date, species, "HS", hatching_success, "%, ES", emergence_success, "%"),
                    group = "Nests") %>%
  addLayersControl(baseGroups = c("Aerial", "Place names"),
