...
Using [api tricks](https://www.rstudio.com/resources/videos/using-web-apis-from-r/).

```{r api}
#' Expand an API serializer name into a WAStD API url
wastd_api <- . %>% paste0("https://strandings.dpaw.wa.gov.au/api/1/", .)

# Reqest GET query parameters and authorization headers
qry = list(taxon="Cheloniidae", limit=10000, format="json")
hdr = c(Authorization = Sys.getenv("WASTD_APITOKEN"))

res_trk <- GET(wastd_api("turtle-nest-encounters/"), 
               query = qry, add_headers(.headers = hdr)) %>% stop_for_status()
res_tag <- GET(wastd_api("animal-encounters/"), 
               query = qry, add_headers(.headers = hdr)) %>% stop_for_status()
```

### Formatting the API response
See [purrr talk](https://www.rstudio.com/resources/videos/happy-r-users-purrr/).

```{r data_munging}
#' Parse a HTTP response of GeoJSON to a data.table of features and handle errors
parse_geojson <- function(res){
  text <- content(res, as = "text", encoding = "UTF-8")
  if (identical(text, "")) stop("The response did not return any content.")
  fromJSON(text, flatten=T)$results$features
}

# Datetime
utc_as_gmt08 <- . %>% parse_date_time(orders = c("YmdHMSz", "adbYHMS")) %>% with_tz(tzone="Australia/Perth")
turtle_date <- . %>% as_date %>% -hours(12) %>% as_date

#' A filter function to restrict data to THV from 19 Dec 2016
thv_filter <- . %>% filter(
    latitude < -21.44,
    latitude > -21.47,
    longitude > 114.96,
...
tracks <- res_trk %>% 
  ...
  transmute(
    ...
    longitude=as.numeric(properties.longitude),
    latitude=as.numeric(properties.latitude),
...
tags <- res_tag %>% 
  ...
  transmute(
    ...
    nesting=properties.nesting
  ) %>% thv_filter
save(tags, file="data/tags.Rda")
load("data/tags.Rda")
```

* **`r nrow(tracks)` records** retrieved from WAStD API on 
  `r res_trk$headers$date` when compiling this workbook
* Track observations span **`r min(tracks$datetime)` - `r max(tracks$datetime)`"*
* **`r nrow(tags)` records** retrieved from WAStD API on 
  `r res_tag$headers$date` when compiling this workbook
...
leaflet(tracks) %>%
  ...
  addAwesomeMarkers(~longitude, ~latitude, data=tags, label=~species,
                    icon=tag,
                    popup=~paste(health, species, "nesting:", nesting, date))
```
...
leaflet() %>% 
  addTiles() %>% 
  addPolygons(data=grd, weight=1, color="orange", label = ~paste(id))
```
...
d.sp <- SpatialPoints(coords=dplyr::select(nesting_encounters, longitude, latitude), proj4string=wgs84)
...
all_nesting_by_date <- nesting_encounters %>%
  group_by(date, species) %>% tally(sort=F) %>% ungroup() %>% rename(all=n)
...
gg_nesting_distribution <- ggplot(site_summary, aes(xmin, ymin, z=n)) + 
  facet_wrap("species") + 
...
gg_nesting_distribution
ggsave(gg_nesting_distribution, file="data/nesting_distribution_all_time.pdf", width=7, height=5)
```
