...
## QA
```{r orphaned_tracks}
tracks_thv_orph <- tracks_orph %>% filter_thv()
tracks_di_orph <- tracks_orph %>% filter_di()
tracks_ri_orph <- tracks_orph %>% filter_ri()

# Surveys are missing for each row in:
tracks_orph %>%
  dplyr::group_by(date, site_name) %>%
  tally() %>%
  dplyr::arrange(site_name)

# These tracks have no site:
tracks_all %>% 
  filter(is.na(site_id)) %>%
  add_nest_labels() %>% map_tracks()
# Actions: 
# Extend site polygons to include GPS error
# Add sites for new surveyed beaches > new report?
# Add sites for known training areas
# Delete training records outside training areas
```
