...
#### 2018-19
The map (once enabled) is saved as a PNG file `r glue::glue(prefix_pth, "_tagged_nests.png")`.
```{r nests_all, eval=F}
nests_pth %>% 
  filter_2018() %>%
  map_nests() %>%
  mapview::mapshot(file = glue::glue("{prefix_pth}_tagged_nests.png"))
```
...
### All beaches
#### 2017-18
```{r tracks_all_sectors_2017}
tracks_pth %>% filter_2017() %>% nesting_type_by_season_species() %>% kable()
```

#### 2018-19
```{r tracks_all_sectors_2018}
tracks_pth %>% filter_2018() %>% nesting_type_by_season_species() %>% kable()
```
...
#### 2017-18
```{r tracks_cem_2017}
tracks_pth_cem %>% filter_2017() %>% nesting_type_by_season_species() %>% kable()
```

#### 2018-19
```{r tracks_cem_2018}
tracks_pth_cem %>% filter_2018() %>% nesting_type_by_season_species() %>% kable()
```

### Pretty Pool Beach
#### 2017-18
```{r tracks_ppo_2017}
tracks_pth_ppo %>% filter_2017() %>% nesting_type_by_season_species() %>% kable()
```

#### 2018-19
None yet.
```{r tracks_ppo_2018, eval=FALSE}
tracks_pth_ppo %>% filter_2018() %>% nesting_type_by_season_species() %>% kable()
```
...
tracks_ts(tracks_pth_cem, surveys_pth_ppo, "Pretty Pool Beach", prefix_pth)
...
* There were **`r surveys_pth %>% filter_2018() %>% nrow` surveys** 
  including training for a total duration of 
  **`r sum(filter_2018(surveys_pth_prod)$duration_hours)` hours**.
* At Cemetery Beach, **`r survey_ground_covered(surveys, 35, 1.6, season=2018)` km** were walked 
  in **`r survey_count(surveys_pth, 35, season=2018)`** surveys.
* At Pretty Pool Beach, **`r survey_ground_covered(surveys, 45, 1.8, season=2018)` km** were walked 
  in **`r survey_count(surveys_pth, 45, season=2018)`** surveys.
