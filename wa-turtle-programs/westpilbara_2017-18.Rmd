...
# Data
```{r load_data, eval=T}
tracks_wp <- tracks_all %>% filter_wp() %>% filter_realspecies() %>% add_nest_labels()
# tracks_cw <- tracks_all %>% filter_cw %>% filter_realspecies() # none yet
tracks_bb <- tracks_all %>% filter_bb() %>% filter_realspecies()
tracks_yc <- tracks_all %>% filter_yc() %>% filter_realspecies()

surveys_wp <- surveys %>% filter_wp()
surveys_wp_prod <- surveys_wp %>% filter_realsurveys()
# surveys_cw <- surveys %>% filter_cw
surveys_bb <- surveys %>% filter_bb()
surveys_yc <- surveys %>% filter_yc()
...
# tracks_bb %>% map_tracks()
# tracks_yc %>% map_tracks()
```
...
t <- tracks_all %>%  add_nest_labels() %>% dplyr::filter(is.na(season))
t[1,]$datetime %>% httpdate_as_season()
...
```{r dist_nests}
disturbed_nests_wp <- tracks_wp %>% filter_2017() %>% filter(disturbance == "present")
disturbed_nests_wp %>% filter_2017() %>% map_tracks()
```
...
surveys_wp %>%
  dplyr::select(
    -site_type, -site_id, -reporter_username, -id, -absolute_admin_url
  ) %>%
  dt()
```
