...
tracks_bb <- tracks_wp %>% filter_bb()
tracks_yc <- tracks_wp %>% filter_yc()
...
surveys_wp %>% filter_surveys_missing_end %>% dt()
```
...
tracks_map <- suppressWarnings(
  basemap +
    geom_point(
      aes(longitude, latitude, colour = nest_type),
      data = tracks_wp, 
      # alpha = 0.8
    )
) +
