...
# Data
```{r load_data, eval=T}
tracks_wp <- tracks_all %>% filter_wp() %>% filter_2017() %>% filter_realspecies() %>% add_nest_labels()
...
```{r nests_all, eval=F}
nests_wp %>% map_nests() %>% mapview::mapshot(file=glue::glue("{prefix}_tagged_nests.png"))
```
