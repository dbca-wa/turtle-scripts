...
# QA
This section identifies tracks and disturbances outside known sites and missing surveys.
...
* Add WAStD sites for new surveyed beaches and include in existing or new report.
* Add WAStD sites for known training areas.
...
tracks_all %>%
  ...
  dplyr::tally() %>%
  dplyr::rename(number_of_tracks = n) %>%
...
```{r}
tracks_wp %>% filter(disturbance == "present") %>% add_nest_labels() %>% map_tracks()
```
