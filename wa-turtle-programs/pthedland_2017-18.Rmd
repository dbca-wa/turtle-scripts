...
nests_pth <- nests_all %>% filter_port_hedland_sites() %>% filter_2017()
...
### Tagged nests
Currently showing all tagged nests.
The map is saved as a PNG file `r glue::glue(prefix, "_tagged_nests.png"`.

```{r nests_all, fig.height=9, fig.width=10}
nests_pth %>% map_nests() %T>% 
...
* There were **`r nrow(surveys_pth)` surveys** including training for a total duration
  of **`r sum(surveys_pth_prod$duration_hours)` hours**.
