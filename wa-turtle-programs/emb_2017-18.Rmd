...
```{r, eval=T}
source("load_data.R")
tracks_emb <- tracks_all %>% filter_emb() %>% filter_2017()
tracks_ap <- tracks_all %>% filter_emb_ap() %>% filter_2017()
tracks_cv <- tracks_all %>% filter_emb_cvp() %>% filter_2017()
surveys_emb <- surveys %>% filter_emb() %>% filter_2017()
dist_emb <- disturbance %>% filter_emb() %>% filter_2017()
```
...
```{r data_export}
tracks_emb %>% select(-obs, -photos) %>% readr::write_csv("../data/tracks_emb.csv")
surveys_emb %>% readr::write_csv("../data/surveys_emb.csv")
dist_emb %>% select(-photos) %>% readr::write_csv("../data/dist_emb.csv")
```
