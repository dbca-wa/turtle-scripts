...
if (refresh_data) {
  ...
  unzip(tmp, "ningaloov6.mdb", exdir = here("ningaloo", "data"))
  unlink(tmp)
}
db <- here("ningaloo", "data", "ningaloov6.mdb")
...
sites %>% readr::write_csv(here("ningaloo", "data","sites.csv"))
```
...
rgdal::writeOGR(polys.df, 
                here("ningaloo", "data", "sites.geojson"), 
                layer="geojson", 
                driver="GeoJSON", 
...
surveys %>% readr::write_csv(here("ningaloo", "data","surveys.csv"))
...
crawls %>% readr::write_csv(here("ningaloo", "data","crawls.csv"))
```
...
nests %>% readr::write_csv(here("ningaloo", "data","nests.csv"))
# DT::datatable(nests)
```
...
summary_nests <- tbl_df(nests) %>%
  ...
  inner_join(surveys)
summary_nests %>% readr::write_csv(here("ningaloo", "data","summary_nests.csv"))
...
summary_nests_seasons %>% readr::write_csv(here("ningaloo", "data","summary_nests_seasons.csv"))
...
summary_crawls <- tbl_df(crawls) %>%
  ...
  inner_join(surveys) %>%
  select(-x_max, -x_min, -y_max, -y_min)
summary_crawls %>% readr::write_csv(here("ningaloo", "data","summary_crawls.csv"))
...
```{r}
ckanr::resource_update(Sys.getenv("ETL_RID"), "ningaloo_etl.html")
ckanr::resource_update(Sys.getenv("SITES_RID"), path=here("ningaloo", "data", "sites.geojson"))
ckanr::resource_update(Sys.getenv("SITES_CSV_RID"), path=here("ningaloo", "data", "sites.csv"))
ckanr::resource_update(Sys.getenv("SURVEYS_RID"), path=here("ningaloo", "data", "surveys.csv"))
ckanr::resource_update(Sys.getenv("CRAWL_RID"), path=here("ningaloo", "data", "crawls.csv"))
ckanr::resource_update(Sys.getenv("NEST_RID"), path=here("ningaloo", "data", "nests.csv"))
ckanr::resource_update(Sys.getenv("NEW_NEST_RID"), path=here("ningaloo", "data", "summary_nests.csv"))
ckanr::resource_update(Sys.getenv("NEST_SEASON_RID"), path=here("ningaloo", "data", "summary_nests_seasons.csv"))
ckanr::resource_update(Sys.getenv("FALSE_CRAWL_RID"), path=here("ningaloo", "data", "summary_crawls.csv"))
```
